!window.qcVideo && function (global) {
                    ...
                    if (_config.report) {
                        for (; _error.length;) {
                            ...
                            if (_isOBJByType(_config.ignore, "Array"))
                                for (var i = 0, l = _config.ignore.length; l > i; i++) {
    ...
    function () {
        ...
        }, util.loadScript = function (src, callback, attrs, remove) {
            var tag = document.createElement("script");
            if (tag.onload = tag.onreadystatechange = function () {
                    this.readyState && "loaded" !== this.readyState && "complete" !== this.readyState || ("function" == typeof callback && callback(), tag.onload = tag.onreadystatechange = null, tag.parentNode && !remove && tag.parentNode.removeChild(tag))
                }, attrs)
                for (var k in attrs)
                    if (attrs.hasOwnProperty(k)) {
                        var attr = attrs[k];
                        null === attr ? tag.removeAttribute(attr) : tag.setAttribute(k, attr)
                    } tag.src = src, document.getElementsByTagName("head")[0].appendChild(tag)
        }, util
        ...
        return {
            ...
            ERROR_MSG: {
                10000: "璇锋眰瓒呮椂,璇锋鏌ョ綉缁滆缃�",
                10001: "鏁版嵁瑙ｆ瀽澶辫触",
                10002: "杩炴帴瓒呮椂锛岃绋嶅悗鍐嶈瘯",
                11046: "瀵嗙爜閿欒锛岃閲嶆柊杈撳叆",
                1003: "瀵嗙爜閿欒锛岃閲嶆柊杈撳叆",
                10008: "瑙嗛婧愪笉瀛樺湪"
            },
            TIP_MESSAGE: {
                LoadVideoFailed: "鍔犺浇瑙嗛澶辫触",
                VideoNeedTranscode: "缂哄皯瑙嗛鏁版嵁锛岃瀵硅棰戣繘琛岃浆鐮�"
            },
            NAMES_DEFINITION: {
                original: "瓒呮竻",
                high: "楂樻竻",
            ...
            UNICODE_WORD: {
                ...
                VOLUME: "闊抽噺",
                FULL_SCREEN: "鍏ㄥ睆",
                EXIT_FULL_SCREEN: "閫€鍑哄叏灞�",
                TIP_OPR_VOLUME: "鎴栫敤閿洏鈫� 閿洏鈫�",
                SETTING: "璁剧疆",
                TIP_REQUIRE_FLASH: "褰撳墠娴忚鍣ㄤ笉鑳芥敮鎸佽棰戞挱鏀撅紝鍙笅杞芥渶鏂扮殑QQ娴忚鍣ㄦ垨鑰呭畨瑁匜LASH鍗冲彲鎾斁",
                TIP_UPDATE_FLASH: "褰撳墠娴忚鍣╢lash鐗堟湰杩囦綆锛屽彲涓嬭浇鏈€鏂扮殑FLASH鐗堟湰杩涜鎾斁",
                TIP_CLICK_UPDATE_FLASH: "鐐瑰嚮鏇存柊",
                SWITCH_DEF: "鏇存崲娓呮櫚搴�",
                NET_ERROR: "Error: 瑙嗛鍔犺浇澶辫触,鐐瑰嚮鎾斁鎸夐挳閲嶆柊鎾斁"
            },
            FIRE: "FIRE",
            EVENT: {
        ...
        return {
            NET_ERROR: constants.UNICODE_WORD.NET_ERROR
        }
        ...
        for (apiMap = [
            ...
            for (fullscreenAPI.supportFullScreen = !0, i = 0; i < browserApi.length; i++) fullscreenAPI[specApi[i]] = browserApi[i];
        ...
        return Base.extend({
            verifyDone: function (data) {
                ...
                h5player.render(me.store), me.sdklink.setSwf(h5player.mediaPlayer),
                    delete me.sdklink
            },
            ...
            erTip: function (msg, pwdEr) {
                if (pwdEr) this.$pwd.find(".txt").text(msg).css("visibility", "visible");
                else {
                    var listeners = this.sdklink.listeners;
                    listeners && listeners.playStatus && listeners.playStatus("error", msg), this.option.hide_h5_error || alert(msg)
                }
            },
            sureHandler: function () {
                var me = this,
                    ...
                    able = pwd.length > 0;
                $pwd.find(".txt").text(able ? "" : "鎶辨瓑锛屽瘑鐮侀敊璇�").css("visibility", able ? "hidden" : "visible"), able && me.askDoor(!1, pwd)
            },
        ...
        return Base.extend({
            ...
            verifyDone: function (targetId, opt, listener) {
                ...
                }), link.modeName = "Html5", TDBankReporter.pushEvent("connectInitH5", {
                    setting: {
                        app_id: opt.app_id
                    }
                });
                else {
                    if ("flash" != ver) return ele.innerText = constants.UNICODE_WORD.TIP_REQUIRE_FLASH, void(listener && listener.playStatus && listener.playStatus("error", {
                        code: -1,
                        message: "env error"
                    }));
            ...
            constructor: function (targetId, opt, listener) {
                ...
                if (util.loadScript("//pingjs.qq.com/h5/stats.js?v2.0.4", null, {
                        name: "MTAH5",
                        sid: "500407535",
                        cid: "500407537"
                    }, !0), ableReportJsError = 0 === ableReportJsError && opt && opt.app_id in constants.BAD_JS_REPORT_WHITE_APP_IDS ? 1 : -1, 1 === ableReportJsError && BJ_REPORT.init({
                        id: 1067,
                        uin: 123,
                        combo: 0,
                        delay: 1e3,
                    ...
                    if (video) return video.urls ? this.verifyDone(targetId, opt, listener) : alert("缂哄皯瑙嗛鍦板潃淇℃伅锛岃琛ラ綈urls锛�");
                    alert("缂哄皯鍙傛暟锛岃琛ラ綈appId锛宖ile_id")
                } else alert("娌℃湁鎸囧畾鏈夋晥鎾斁鍣ㄥ鍣紒")
            },
            remove: function () {
        ...
        var global = window,
            ...
            SwfJsLink = function (id, listeners) {
                ...
                listeners && "function" != type ? "object" == type && util.merge(me.listeners, listeners) : me.listeners.playStatus = listeners || function () {}, global[id + "_callback"] = function (cmd) {
                    var cmds = cmd.split(":"),
                        key = cmds[0];
                    if (me.listeners.hasOwnProperty(key)) switch (key) {
                        case "playStatus":
                            me.listeners[key](cmds[1]), "ready" == cmds[1] && ("Flash" == me.modeName ? MtaH5.clickStat("flash") : MtaH5.clickStat("h5"));
                            break;
                        case "fullScreen":
        ...
        return Base.instance({
            ...
            render: function (opt) {
                ...
                var hasMoreFormat = !1,
                    sourceIsMp4 = !1,
                    hasDefinition = !1,
                    defConfigs = constants.DEFINITION_PRIORITY,
                ...
                if ($.each(video, function (_, item) {
                        ...
                        var _defList = "";
                        for (var i in defConfigs) _defList = "," + defConfigs[i].join(",") + ",", -1 != _defList.indexOf("," + def + ",") && (hasDefinition = !0);
                        store.videos[def] = {
                            definition: def,
                            name: fileInfo.name || "",
        ...
        return UICom.extend({
            ...
            _click_progress_handler: function (event) {
                if (this.processDrag.isAble()) {
                    ...
                    this.debug("鐐瑰嚮鎾斁杩涘害鐧惧垎姣�==" + percent), this.fire(FIRE, {
                        event: EVENT.UI_DRAG_PLAY,
                        value: {
                            percent: percent
                        }
        ...
        return UICom.extend({
            ...
            init: function () {
                ...
                this.$el.append(tplObj.setting_definition({
                    name: constants.UNICODE_WORD.SWITCH_DEF,
                    values: data
                }))
            },
            show: function (obj) {
        ...
        return Base.extend({
            ...
            constructor: function (info) {
                ...
                video.url || console && console.error("褰撳墠鐜娌℃湁鍙挱鏀剧殑瑙嗛鏂囦欢");
            ...
            rememberPlay: function (status, system, duration) {
                ...
                lVolume > 0 && system.volume(lVolume), lPlayed > 0 && (0 == duration || duration > lPlayed + 5) && (this.whenReadyToPlay = !0, this.debug("璁板繂鎾斁浣嶇疆:", lPlayed), this.system.setUrl(this.video_data.url), this.system.play())
            },
            switchClarity: function (no) {
                ...
                } else me.debug("褰撳墠涓嶆敮鎸佽娓呮櫚搴﹀垏鎹�", no)
            },
        ...
        return Base.extend({
            ...
            getAllDefinition: function () {
                ...
                for (def in this.store.videos)
                    for (ind in pir) $.each(pir[ind], function (_, i) {
            ...
            isInDefinition: function (no) {
                if (no !== undefined) {
                    var all = this.getAllDefinition();
                    for (var k in all)
                        if (all[k].resolutionNameNo == no) return !0
                }
                return !1
            },
                    ...
                    getRstFromDefPriority = function (res) {
                        ...
                        if (arr && arr.length > 0)
                            for (var i = 0; i < arr.length; i++)
                                if (videos[arr[i]]) return videos[arr[i]]
                    },
                    all = this.getAllDefinition();
                if (me.debug(":get video:resource==" + res + ";definition==" + def), def !== undefined && (rst = videos[0 | def], all[def] || (rst = null)), rst || res === undefined || (rst = getRstFromDefPriority(0 | res)), !rst)
                    for (var i = 0, j = resolutionPriority.length; j > i && !(rst = getRstFromDefPriority(resolutionPriority[i])); i++);
                if (rst) {
                    var map = me.getAllDefinition()[rst.definition];
                    return me.debug(":getVideoInfo result:娓呮櫚搴�==" + map.resolution + ";鍚嶇О==" + map.resolutionName + ";sdk娓呮櫚搴�" + map.resolutionNameNo), util.merge({
                    ...
                    if (version.IS_MOBILE && height > 0) {
                        ...
                        rate != askRate && (zoom = height * askRate / sourceH), this.children.Bottom_container.$el.css("zoom", zoom), setTimeout(function () {
                            var h = $settingEl.height(),
                                ...
                                rate = h / ch,
                                size = "1rem";
                            .3 > rate ? size = "2rem" : .5 > rate ? size = "1.5rem" : rate > .9 && (size = "0.5rem"), me.log(h, height, ch, rate, size), $settingEl.css("font-size", size).height("100%").hide()
                        }, 300)
